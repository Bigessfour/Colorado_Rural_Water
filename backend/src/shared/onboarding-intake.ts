...
export function mergeOnboardingIntake(
  ...
  if (stepRaw !== undefined && stepRaw !== null && stepRaw !== '') {
    ...
    if (!Number.isFinite(s) || s < 0 || s >= ONBOARDING_STEPS.length) {
      return { ok: false, error: `currentStep must be 0–${ONBOARDING_STEPS.length - 1}` };
    }
  ...
  if (pathRaw !== undefined && pathRaw !== null && pathRaw !== '') {
    if (typeof pathRaw !== 'string' || !PATHS.includes(pathRaw as OnboardingPath)) {
  ...
  if (unitRaw !== undefined && unitRaw !== null && unitRaw !== '') {
    if (typeof unitRaw !== 'string' || !UNITS.includes(unitRaw as ReadingUnit)) {
  ...
  if (schedRaw !== undefined && schedRaw !== null && schedRaw !== '') {
    if (typeof schedRaw !== 'string' || !SCHEDULES.includes(schedRaw as ReadSchedule)) {
  ...
  if (fmtRaw !== undefined && fmtRaw !== null && fmtRaw !== '') {
    if (typeof fmtRaw !== 'string' || !EXPORT_FORMATS.includes(fmtRaw as ExportFormatHint)) {
      return { ok: false, error: 'exportFormat must be csv, xlsx, both, or unknown' };
    }
  ...
  const intake: OnboardingIntake = {
    ...
    systemName: 'systemName' in body ? trimStr(body.systemName, 120) : base.systemName,
    serviceTerritoryAddress:
    ...
    primaryContactName:
      'primaryContactName' in body ? trimStr(body.primaryContactName, 80) : base.primaryContactName,
    primaryContactEmail:
      'primaryContactEmail' in body ? trimStr(body.primaryContactEmail, 128) : base.primaryContactEmail,
    primaryContactPhone:
      'primaryContactPhone' in body ? trimStr(body.primaryContactPhone, 32) : base.primaryContactPhone,
    billingClerkName:
      'billingClerkName' in body ? trimStr(body.billingClerkName, 80) : base.billingClerkName,
    billingClerkPhone:
      'billingClerkPhone' in body ? trimStr(body.billingClerkPhone, 32) : base.billingClerkPhone,
    meterCountEstimate:
      'meterCountEstimate' in body ? optionalCount(body.meterCountEstimate) : base.meterCountEstimate,
    sourceCountEstimate:
      'sourceCountEstimate' in body ? optionalCount(body.sourceCountEstimate) : base.sourceCountEstimate,
    readSchedule,
    preferredUnit,
    billingCycleNote:
      'billingCycleNote' in body ? trimStr(body.billingCycleNote, 200) : base.billingCycleNote,
    municipalBillingSystem:
    ...
    exportFormat,
    exportColumnHints:
      'exportColumnHints' in body ? trimStr(body.exportColumnHints, 500) : base.exportColumnHints,
    onboardingPath,
    hasHistoricalExport:
      'hasHistoricalExport' in body ? Boolean(body.hasHistoricalExport) : base.hasHistoricalExport,
    historyNotes: 'historyNotes' in body ? trimStr(body.historyNotes, 500) : base.historyNotes,
    updatedAt: now,
  };
  ...
  if (markComplete) {
    ...
    if (!intake.primaryContactEmail.trim()) {
      return { ok: false, error: 'primaryContactEmail is required to complete intake' };
    }
  }

...
function loc(
  partial: Partial<MeterLocation> & Pick<MeterLocation, 'tenantId' | 'meterId' | 'serviceAddress'>,
): MeterLocation {
  return {
    occupantName: null,
    accountNumber: null,
...
function rdg(
  partial: Partial<MeterReading> &
    Pick<MeterReading, 'tenantId' | 'meterId' | 'timestamp' | 'cumulativeReading'>,
): MeterReading {
  return {
...
describe('parseMeterCreateBody', () => {
  it('requires meterId and serviceAddress', () => {
    ...
    assert.equal(parseMeterCreateBody('t1', { serviceAddress: '1 Main' }).ok, false);
  });
...
describe('MemoryMeterStore inventory CRUD isolation', () => {
  it('lists only the caller tenant meters', async () => {
    ...
    await store.putLocation(loc({ tenantId: 'a', meterId: '1', serviceAddress: 'A St' }));
    await store.putLocation(loc({ tenantId: 'b', meterId: '1', serviceAddress: 'B St' }));
  ...
  it('delete cascades readings for that meter only', async () => {
    const store = new MemoryMeterStore();
    await store.putLocation(loc({ tenantId: 'a', meterId: '1', serviceAddress: 'A St' }));
    await store.putLocation(loc({ tenantId: 'a', meterId: '2', serviceAddress: 'B St' }));
    await store.putReading(
      rdg({ tenantId: 'a', meterId: '1', timestamp: '2026-07-01T00:00:00.000Z', cumulativeReading: 10 }),
    );
    await store.putReading(
      rdg({ tenantId: 'a', meterId: '1', timestamp: '2026-07-15T00:00:00.000Z', cumulativeReading: 20 }),
    );
    await store.putReading(
      rdg({ tenantId: 'a', meterId: '2', timestamp: '2026-07-01T00:00:00.000Z', cumulativeReading: 5 }),
    );
    await store.putReading(
      rdg({ tenantId: 'b', meterId: '1', timestamp: '2026-07-01T00:00:00.000Z', cumulativeReading: 99 }),
    );

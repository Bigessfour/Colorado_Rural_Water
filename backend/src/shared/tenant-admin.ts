...
export function normalizeTenantId(
  ...
  if (tenantId.startsWith("_")) {
    return {
      ok: false,
      error: "tenantId cannot start with underscore (reserved)",
    };
  }
...
export function normalizeMeterCountEstimate(
  ...
  if (!Number.isFinite(n) || n < 0 || n > 1_000_000) {
    return {
      ok: false,
      error: "meterCountEstimate must be a number between 0 and 1000000",
    };
  }
...
export function normalizeMapCenterFields(
  ...
  const hasLat =
    "mapCenterLat" in body &&
    body.mapCenterLat !== undefined &&
    body.mapCenterLat !== "";
  const hasLng =
    "mapCenterLng" in body &&
    body.mapCenterLng !== undefined &&
    body.mapCenterLng !== "";
  if (hasLat !== hasLng) {
    return {
      ok: false,
      error:
        "mapCenterLat and mapCenterLng must be set together (or both omitted)",
    };
  }
  ...
  if (!Number.isFinite(lat) || lat < -90 || lat > 90) {
    return {
      ok: false,
      error: "mapCenterLat must be a number between -90 and 90",
    };
  }
  if (!Number.isFinite(lng) || lng < -180 || lng > 180) {
    return {
      ok: false,
      error: "mapCenterLng must be a number between -180 and 180",
    };
  }

  let mapZoom: number | null = 12;
  if (
    body.mapZoom !== undefined &&
    body.mapZoom !== null &&
    body.mapZoom !== ""
  ) {

...
export function recommendedActionForRow(row: Pick<WorkOrderRow, 'mode' | 'type' | 'summary'>): string {
  if (row.mode === 'Watch') {
...
export function buildWorkOrderMapLink(meterId: string, appBaseUrl?: string): string {
...
export function buildWorkOrdersXlsxBuffer(rows: WorkOrderRow[]): Buffer {
  ...
  const ws = XLSX.utils.json_to_sheet(sheetRows, { header: [...WORK_ORDER_HEADERS] });

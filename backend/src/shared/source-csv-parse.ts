...
export function guessSourceColumnMapping(
  headers: string[],
): SourceColumnMapping {
...
export function applySourceMapping(
  ...
  if (!mapping.sourceName && !mapping.sourceId) {
    errors.push(
      "We could not find a Source Name or Source ID column. Please map it.",
    );
  }
  if (!mapping.timestamp)
    errors.push("We could not find a read date / timestamp column.");
  if (!mapping.periodVolume && !mapping.cumulativeReading) {
    errors.push(
      "We could not find a period volume or cumulative reading column.",
    );
  }
  ...
  parsed.rows.forEach((raw, idx) => {
    ...
    if ((!sourceName && !sourceIdRaw) || !tsRaw || (!periodRaw && !cumRaw)) {
      warnings.push(
        `Row ${lineNo}: skipped — missing source, date, or volume.`,
      );
      return;
    }
    ...
    if (!Number.isFinite(value)) {
      warnings.push(
        `Row ${lineNo}: skipped — volume "${valueRaw}" is not a number.`,
      );
      return;
    }
    ...
    if (!timestamp) {
      warnings.push(
        `Row ${lineNo}: skipped — could not understand date "${tsRaw}".`,
      );
      return;
    }
    ...
    if (typeRaw && !sourceType) {
      warnings.push(
        `Row ${lineNo}: unknown source type "${typeRaw}" — will use well if creating.`,
      );
    }
...
export function parseSourceReadingsCsv(
  ...
  const safeOverride: SourceColumnMapping = {};
  const dropped: string[] = [];
  if (mappingOverride) {
    for (const [field, col] of Object.entries(mappingOverride) as [
      SourceCanonicalField,
      string | undefined,
    ][]) {
      if (!col) continue;
      if (headerSet.has(col)) safeOverride[field] = col;
  ...
  if (dropped.length) {
    result.warnings.unshift(
      `Ignored saved source column map for missing headers (${dropped.join(", ")}); guessed from this file instead.`,
    );
  }

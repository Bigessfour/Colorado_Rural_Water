...
/**
 * Named sources CRUD — Spec §7a / ticket G1 (Kelly demo: name wells before source ingest).
...
export const handler: AuthedHandler = async (event) => {
  ...
  try {
    ...
    if (method === "POST" && !sourceIdParam) {
      ...
      const existing = await store.getSource(
        tenantId,
        normalized.source.sourceId,
      );
      if (existing) {
        return badRequest(
          `Source ${normalized.source.sourceId} already exists — use PUT to update`,
        );
      }
    ...
    if (method === "PUT" && sourceIdParam) {
      ...
      if (!existing)
        return json(404, { error: `Source ${sourceIdParam} not found` });
    ...
    return badRequest(
      err instanceof Error ? err.message : "Sources request failed",
    );
  }
};

...
| Meter locations             | `sk=LOC#{meterId}`; **service address stable**; occupant name mutable; optional asset fields (manufacturer, model, serialNumber, meterSize, installDate, meterType, locationDetail, radioId, lastTestedAt, notes); optional WGS84 `latitude`/`longitude` for map pins (Feature 011); **DELETE `/meters/{meterId}`** removes `LOC#` and cascades all `RDG#{meterId}#…` for that tenant only (mirrors SRC→SRD cascade) |
...
| Alert status (C3)           | `sk=ALERT#STATUS#{alertId}`; acknowledged/dispatched/resolved + note + audit. `meterId`/`summary` from evaluated alerts (client meterId ignored).                                                                                                                                                                                                                                                                    |
| Alert activity (C3)         | `sk=ALERT#EVT#{meterId}#{createdAt}#{alertId}`; meter timeline only (balance skips activity). Status+activity via Dynamo `TransactWrite` when both apply. On `GET /meters/{id}`.                                                                                                                                                                                                                                     |
...
| Kelly Review (F5)           | Synthetic `pk=TENANT#_review` (stays under LeadingKeys); `sk=SESSION#{id}` + `SESSION#{id}#STEP#{stepId}`; not municipal data. SES summary on submit when `REVIEW_NOTIFY_TO` / `REVIEW_FROM_EMAIL` set.                                                                                                                                                                                                              |

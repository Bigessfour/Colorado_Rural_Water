...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Non-technical user can ingest a messy **customer** file with friendly guidance                                                            | Upload `sample-data/Town_of_Steve_Meter_Export_MESSY.xlsx` (CSV also OK); mapper; Mapped vs unused; import/skip summary | [x] 2026-08-04 Steve F1 — CSV path; Excel-first copy + skip counts 2026-08-04 hardening                                                                                |
...
## Ingest paths (sync vs S3)

| Path               | When                                                         | How                                                                                             |
| ------------------ | ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| **Sync Upload UI** | Monthly / demo Excel or CSV with visual mapper + Check first | SPA → `POST /ingest` (`excelBase64` / `csvText`). Prefer Excel. Cap ~5 MiB.                     |
| **S3 presign**     | Ops bulk drop when mapping already saved (or auto-guess OK)  | `DEMO_USER=… DEMO_PASS=… ./scripts/smoke-presign-ingest.sh sample-data/messy-readings-july.csv` |

Kelly demo uses the sync path. Do **not** expect the SPA mapper to auto-route large files to S3 yet (Pilot).

**Ops note (2026-08-04):** Presign + PutObject verified for `messy-readings-july.csv`. `s3-ingest` initially failed with missing `s3:ListBucket` on the uploads bucket — **fixed + Terraform applied** (`UploadsListBucket` with `tenants/*` prefix). Re-run `./scripts/smoke-presign-ingest.sh` after apply.
